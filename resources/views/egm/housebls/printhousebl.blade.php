...
<header>
    <h6 id="companyName" class=""> QC Logistics Limited </h6>
    ...
    <small style="position: absolute; left: 700px; top: 15px">
        ...
        <span style="margin-left: 10px"> ETA: <strong> {{$masterbl->arrival ? date('d-m-Y', strtotime($masterbl->arrival)) : null}} </strong></span>
    </small>
    ...
    @if($masterbl->pucode == "BDKAM" || $masterbl->pucode == "BDPNG")
        <h2 style="margin-top: -3px; text-align: center; text-transform: uppercase"> Container Bound for {{$masterbl->puname}} </h2>
    @endif
</header>
        ...
        @foreach($masterbl->housebls as $key => $housebl)
            ...
            @foreach($housebl->containers as $container)
                <tr>
                    ...
                    <td rowspan="{{$totalRow}}"> {{$housebl->exportername}} {{$housebl->exporteraddress}} </td>
                    <td rowspan="{{$totalRow}}"> {{$housebl->consigneebin}}<br> {{$housebl->consigneename}}<br> {{$housebl->consigneeaddress}} </td>
                    <td rowspan="{{$totalRow}}"> {{$housebl->notifybin}}<br> {{$housebl->notifyname}}<br> {{$housebl->notifyaddress}} </td>
                    ...
                    <td>
                        @if($housebl->dg)
                            <span style="background: #000; color: #fff; padding: 5px; display: block; text-align: center"> {{$housebl->dg ? "DG" : null}} </span> <br>
                        @endif
