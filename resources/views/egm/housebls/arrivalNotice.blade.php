...
<head>
    ...
    <style>
        ...
        #ArrivalSummary table, #ArrivalSummary table th, #ArrivalSummary table td{
            border: 1px solid;
...
<header class="overFlow">
    ...
    <div id="brand" style="margin-top: -80px" class="textCenter">
        ...
        <p ><small> C&F Tower, 4th Floor, 1222 Sk. Mujib Road, Agrabad, Chittagong. Tel : +880-31-2515926-7, 721659.</small></p>
    </div>
    <hr width="100%">
</header>
...
<div class="addressArea">
    ...
    <div class="mrTB">
        <p>Dear Sir(s), </p>
        <p >We are pleased to inform you that above feeder vessel is scheduled to arrive at <strong>{{$housebl->masterbl->puname}}</strong>
            on or about <b> {{$housebl->masterbl->arrival ? date('d-m-Y', strtotime($housebl->masterbl->arrival)) : "---"}} </b> with the under noted cargo on your account. </p>
    </div>
</div> <!-- end contentArea -->
        ...
        <tr>
            ...
            @if ($housebl->containernumber<=5)
                ...
                @foreach($housebl->containers as $container)
                    <strong> {{$container->contref}}/ {{$container->type}} - {{$container->status}} </strong>,
                @endforeach
            </td>
            @else
...
@if($housebls->isNotEmpty())
    <div id="ArrivalSummary">
        <div id="brand" class="textCenter">
            <h1 id="companyName" style="line-height: 25px"> QC Logistics Limited </h1>
            <p><small> C&F Tower, 4th Floor, 1222 Sk. Mujib Road, Agrabad, Chittagong. Tel : +880-31-2515926-7, 721659.</small></p>
            <h2 style="border: 1px solid; padding: 5px; border-radius: 5px; width: 300px; margin: 10px auto"> LIST OF ARRIVAL NOTICE </h2>
        </div>


        <div>
            <p style="float: left"> <strong> F/VSL & Voyage  </strong> : {{$housebls->first()->masterbl->fvessel}} V.{{$housebls->first()->masterbl->voyage}} </p>
            <p style="float: right"> <strong> M/VSL & Voyage  </strong> : {{$housebls->first()->masterbl->mv}}  </p>
        </div>
