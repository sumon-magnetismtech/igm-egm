...
<div id="infoArea" class="mainArea">
    <div class="textCenter">
        <h2 class="textUpper" style="border-radius: 7px; padding: 5px 15px; border:1px solid #000; display: inline-block; margin-left: 33%"> Money Receipt </h2>
    </div>
    <div class="pullLeft" style="width: 510px;">
        <p> SL NO.<strong> QCLOGFRD-{{$data->id}}{{$data->extension_no ? "-EX-$data->extension_no" : null}}</strong></p>
        <p class="my-10"> Received with thanks from M/s. <strong class="textUpper"> {{$data->client_name}}</strong></p>
    ...
    <div class="pullRight" style="width: 200px; margin-top:-10px">
        <p> Issue Date :<span> {{$data->issue_date ? date('d-M-Y', strtotime($data->issue_date)) : null}} </span></p>
        <p> Berthing Date :<span> {{$data->houseBl->masterbl->berthing ? date('d-M-Y', strtotime($data->houseBl->masterbl->berthing)) : null}} </span></p>
    ...
    <table id="billingTable">
        ...
        <tr>
            <td class="">
                ...
                <p class="my-5"> Vessel: <strong>{{$data->houseBl->masterbl->fvessel}} </strong> V. <strong>{{$data->houseBl->masterbl->voyage}}</strong></p>
                <p class="my-5">IMP. REG. No: <strong>{{$data->houseBl->masterbl->rotno}}</strong></p>
                <p class="my-5"> Quantity: <strong>{{$data->quantity}} {{$data->houseBl->packagetype}}</strong> </p>
...
<div id="footerTop">
    ...
    <div id="barcode">
        <br>
        <img src="data:image/png;base64, {{ base64_encode(SimpleSoftwareIO\QrCode\Facades\QrCode::format('png')->size(100)->generate($data->client_name.' '.$data->id.' '.$total)) }} ">
    </div>
</div>
