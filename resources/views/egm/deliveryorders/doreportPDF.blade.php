...
<header>
    <h6 style="text-align: center" id="companyName"> QC Logistics Limited (DO Report) </h6>
</header>
<footer>
    <p>
...
<p class="my-0"> Total DO : <strong> {{count($deliveryOrders)}}</strong>; Total NOC : <strong> {{count($noc)}}</strong></p>
        ...
        @if($deliveryOrders->isNotEmpty())
            @foreach($deliveryOrders as $key => $deliveryOrder)
                <tr>
                    ...
                    <td style="width: 3%"> <nobr>DO</nobr> </td>
                    <td style="width:7%"> {{$deliveryOrder->moneyReceipt->houseBl->bolreference}} </td>
                    <td style="width:4%"> {{$deliveryOrder->moneyReceipt->houseBl->packageno}} <br> {{$deliveryOrder->moneyReceipt->houseBl->packagetype}} </td>
                    <td style="width:12%"> {{$deliveryOrder->moneyReceipt->houseBl->masterbl->mv}} </td>
                    <td style="width:12%"> {{$deliveryOrder->moneyReceipt->houseBl->masterbl->fvessel}} V.{{$deliveryOrder->moneyReceipt->houseBl->masterbl->voyage}} </td>
                    <td style="width:6%"> {{$deliveryOrder->moneyReceipt->houseBl->masterbl->arrival ? date('d-m-Y', strtotime($deliveryOrder->moneyReceipt->houseBl->masterbl->arrival)) : null}} </td>
                    <td style="width:11%"> {{$deliveryOrder->moneyReceipt->houseBl->masterbl->principal}} </td>
                    ...
                    <td style="width:6%"> {{$deliveryOrder->issue_date ? date('d-m-Y', strtotime($deliveryOrder->issue_date)) : null}} </td>
                    <td style="width:6%"> {{$deliveryOrder->upto_date ? date('d-m-Y', strtotime($deliveryOrder->upto_date)) : null}} </td>
        ...
        @if($noc->isNotEmpty())
            @foreach($noc as $key => $deliveryOrder)
                <tr>
                    ...
                    <td style="width:4%"> {{$deliveryOrder->houseBl->packageno}}<br> {{$deliveryOrder->houseBl->packagetype}} </td>
                    <td style="width:12%"> {{$deliveryOrder->houseBl->masterbl->mv}} </td>
                    <td style="width:12%"> {{$deliveryOrder->houseBl->masterbl->fvessel}} V.{{$deliveryOrder->houseBl->masterbl->voyage}} </td>
                    <td style="width:6%"> {{$deliveryOrder->houseBl->masterbl->arrival ? date('d-m-Y', strtotime($deliveryOrder->houseBl->masterbl->arrival)) : null}} </td>
