...
@section('content')
    <form action="{{ route('egmMloDoReport') }}" method="get">
        <div class="row px-2">
            ...
            <div class="col-md-2 px-1 my-1 my-md-0" id="fromDateArea"
                style="display: {{ $dateType == 'custom' ? 'block' : 'none' }}">
                <input type="text" id="fromDate" name="fromDate" class="form-control form-control-sm"
                    value="{{ $fromDate ? date('d/m/Y', strtotime($fromDate)) : '' }}" placeholder="From Date"
                    autocomplete="off">
            </div>
            <div class="col-md-2 px-1 my-1 my-md-0" id="tillDateArea"
                style="display: {{ $dateType == 'custom' ? 'block' : 'none' }}">
                <input id="tillDate" class="form-control form-control-sm"
                    value="{{ !empty($tillDate) ? date('d/m/Y', strtotime($tillDate)) : '' }}" type="text"
                    name="tillDate" placeholder="Till Date" autocomplete="off">
            </div>
                        ...
                        @foreach ($deliveryOrders as $key => $deliveryOrder)
                            <tr>
                                ...
                                <td> {{ $deliveryOrder->moneyReceipt->molblInformations->packageno }} <br>
                                    {{ $deliveryOrder->moneyReceipt->molblInformations->packagetype }} </td>
