...
<div class="addressArea">
    ...
    <div class="pullRight">
        <div id="doInfo">
            ...
            <p> ISSUE DATE. <strong>{{$deliveryInfo->DO_Date ? date('d-M-Y', strtotime($deliveryInfo->DO_Date)) : null}}</strong></p>
        </div>
    </div>
</div> <!-- end contentArea -->
...
<div id="tableArea overFlow">
    <table>
        <tr>
            ...
            <td colspan="4"> <strong> BE DATE:</strong> {{$deliveryInfo->BE_Date ? date('d-M-Y', strtotime($deliveryInfo->BE_Date)) : null}}</td>
        </tr>
        <tr>
            <td colspan="7"> <strong>B/L NO:</strong> {{$deliveryInfo->moneyReceipt->bolRef}} </td>
        </tr>
        <tr>
            <td colspan="7"> Please delivery to <strong>{{$deliveryInfo->moneyReceipt->client->cnfagent}}</strong> the under mentioned goods.</td>
        </tr>
        <tr>
            <td colspan="3"> <strong> V. Name:  </strong>{{$deliveryInfo->moneyReceipt->molblInformations->mlofeederInformation->feederVessel}}({{$deliveryInfo->moneyReceipt->molblInformations->mlofeederInformation->voyageNumber}})</td>

            <td colspan="2"> <strong> Arrival DT: </strong> {{$deliveryInfo->moneyReceipt->molblInformations->mlofeederInformation->arrivalDate ? date('d-M-Y', strtotime($deliveryInfo->moneyReceipt->molblInformations->mlofeederInformation->arrivalDate)) : null}} </td>
            <td colspan="2"> <strong> Reg No.</strong> {{$deliveryInfo->moneyReceipt->molblInformations->mlofeederInformation->rotationNo}} </td>
        </tr>
        <tr>
            <td> MARKS & NOS</td>
            <td> QTY </td>
        ...
        @foreach($deliveryInfo->moneyReceipt->molblInformations->blcontainers as $blcontainer)
            <tr>
                @if($loop->first)
                <td rowspan="{{$countContainers}}"> {{$deliveryInfo->moneyReceipt->molblInformations->shippingmark}} </td>
                <td rowspan="{{$countContainers}}"> {{$deliveryInfo->moneyReceipt->molblInformations->packageno}} {{$deliveryInfo->moneyReceipt->molblInformations->package->description}} </td>
                <td rowspan="{{$countContainers}}" style="word-wrap: break-word"> {{$deliveryInfo->moneyReceipt->molblInformations->description}} </td>
                <td rowspan="{{$countContainers}}"> {{$deliveryInfo->moneyReceipt->molblInformations->grosswt}} </td>
                @endif
        ...
        @endforeach
        <tr>
            <td colspan="7"> VALID UP TO :<strong> {{$deliveryInfo->moneyReceipt->uptoDate ? date('d-M-Y', strtotime($deliveryInfo->moneyReceipt->uptoDate)) : null}}</strong> </td>
        </tr>
        <tr>
...
<div id="footerTop">
    <div id="seal">
        <div class="pullRight textCenter">
            <h4 style="margin: 0"> For: QC Logistics Limited </h4>
            <br>
