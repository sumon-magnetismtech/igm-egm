...
@foreach($unloadingGroups as $portName => $unloadingGroup)
    <header style="text-align: center; display: block">
        <h2 style="border: 1px solid #000; padding: 5px; border-radius:5px; display: inline-block; width: 600px; margin: 0 auto">
                            ...
                            <tr>
                               <td class="textCenter"> {{$i++}} </td>
                                ...
                                @if($loop->first)
                                    ...
                                    <td rowspan="{{$totalContainer}}"> {{$tarWt = str_contains($mloblDetails->containerGroup->group, 20) ? 2300 : 3900}}</td>
                                    <td rowspan="{{$totalContainer}}" class="textCenter"> {{$mloblDetails->grosswt + $tarWt}}</td>
                                    <td rowspan="{{$totalContainer}}" class="textCenter"> {{$mloblDetails->pkgno ?? ""}}({{$mloblInfo->package->packagecode}})</td>
    ...
    <div id="seal overFlow">
        <div class="pullRight textCenter">
            <h4 class="textUpper"> For QC Logistics Limited </h4>
            <br><br><br>
