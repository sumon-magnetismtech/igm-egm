    ...
    <form action="" method="get">
        <div class="row">
            <div class="col-md-2 pr-md-1 my-1 my-md-0">
                <input type="text" id="igm" name="igm" value="{{$request->igm ?? null}}" class="form-control form-control-sm" placeholder="IGM" autocomplete="off">
            </div>
            <div class="col-md-3 px-md-1 my-1 my-md-0">
                <input type="text" id="bolreference" name="bolreference" value="{{$request->bolreference ?? null}}" class="form-control form-control-sm" placeholder=" B/L" autocomplete="off">
            </div>
            <div class="col-md-3 px-md-1 my-1 my-md-0">
                <input type="text" id="feederVessel" name="feederVessel" value="{{$request->feederVessel ?? null}}" class="form-control form-control-sm" placeholder="Vessel" autocomplete="off">
            </div>
            <div class="col-md-2 px-md-1 my-1 my-md-0">
                <input type="text" id="voyageNumber" name="voyageNumber" value="{{$request->voyageNumber ?? null}}" list="voyageList" class="form-control form-control-sm" placeholder="Voyage" autocomplete="off">
                ...
                @foreach($mlobl->blcontainers as $container)
                    <tr class="{{$loop->parent->even ? "bg-light" : null}}">
                            ...
                            <td rowspan="{{$totalContainer}}" class="text-center">
                                <div class="icon-btn">
                                    <nobr>
                                        ...
                                        <a href="{{ url('mloblinformations/'.$mlobl->id.'/edit') }}" data-toggle="tooltip" title="Edit" class="btn btn-warning"><i class="fas fa-pen"></i></a>
                                        @endcan
                                    </nobr>
                                </div>
                            </td>
                        ...
                        <td rowspan="{{$totalContainer}}">
                            ...
                            <a target="_blank" class="btn btn-sm bg-success p-2 m-2" href="{{route('mloMoneyReceiptPdf', $mlobl->mloMoneyReceipt->id)}}">MR-{{$mlobl->mloMoneyReceipt->id}}</a>
                            <br>
                        ...
                        <td rowspan="{{$totalContainer}}">
                            @if($mlobl->mloMoneyReceipt)
                                ...
                                <a target="_blank" class="btn btn-sm bg-success p-2 m-2" href="{{route('mloDoPDF', $mlobl->mloMoneyReceipt->deliveryOrder->id)}}">DO-{{$mlobl->mloMoneyReceipt->deliveryOrder->id}}</a>
                                <br>
                        ...
                        <td rowspan="{{$totalContainer}}" class="text-center">
                            <div class="icon-btn">
                                <nobr>
                                    <a href="{{url('mloblinformations/'.$mlobl->id)}}" data-toggle="tooltip" title="Details" class="btn btn-primary"><i class="fas fa-eye"></i></a>
                                    @can('mlo-mloblinformation-edit')
                                    <a href="{{ url('mloblinformations/'.$mlobl->id.'/edit') }}" data-toggle="tooltip" title="Edit" class="btn btn-warning"><i class="fas fa-pen"></i></a>
                                    @endcan

                                    @can('mlo-mloblinformation-delete')
                                    <form action="{{ url('mloblinformations', [$mlobl->id]) }}" method="POST" data-toggle="tooltip" title="Delete" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm delete"><i class="fas fa-trash"></i></button>
                                    </form>
                                    @endcan
                                    <a href="{{url('blinformations/log/'.$mlobl->id)}}" data-toggle="tooltip" title="Log" class="btn btn-dark"><i class="fas fa-history"></i></a>
                                </nobr>
                            </div>
                        </td>
                        @endif
