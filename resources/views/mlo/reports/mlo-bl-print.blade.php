...
<header>
    ...
    @if($status == "bdkam" || $status == "bdpng")
        <h2 style="margin-top: -3px; text-align: center; text-transform: uppercase"> Container Bound for {{strtoupper($status)}} </h2>
    @endif
</header>
        ...
        @foreach($feederinformation->mloblInformation as $key => $blinformation)
            ...
            @foreach($blinformation->blcontainers as $container)
                <tr>
                    ...
                    <td> {{$blinformation->blConsignee->BIN}}<br> {{$blinformation->blConsignee->NAME}}<br> {{$blinformation->blConsignee->ADD1}} </td>
                    <td> {{$blinformation->blNotify->BIN}}<br> {{$blinformation->blNotify->NAME}}<br> {{$blinformation->blNotify->ADD1}} </td>
