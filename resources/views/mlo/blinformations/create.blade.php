...
@section('content')
    @if($formType == 'edit')
        <form action="{{ route('mloblinformations.update', $mloblinformation->id) }}" method="post" id="mainForm" class="custom-form" enctype="multipart/form-data" >
            @method('PUT')
            <input type="hidden" name="id" value="{{$mloblinformation->id}}">
    @else
        <form action="{{ route('mloblinformations.store') }}" method="post" id="mainForm" class="custom-form" enctype="multipart/form-data" >
    @endif
        @csrf
