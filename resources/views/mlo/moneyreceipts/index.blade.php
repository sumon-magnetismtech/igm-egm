...
@section('breadcrumb-button')
    ...
    <a href="{{ route('mlomoneyreceipts.create') }}" class="btn btn-out-dashed btn-sm btn-success"><i class="fa fa-plus"></i></a>
    @endcan
@endsection
    ...
    <form action="" method="get">
        <div class="row">
            <div class="col-md-2 pr-md-1 my-1 my-md-0">
                <input type="text" id="id" name="id" class="form-control form-control-sm" value="{{$request->id ?? null}}"  placeholder="Money Receipt ID">
            </div>
            ...
            @foreach($moneyReceipts as $key => $moneyReceipt)
                ...
                @foreach($moneyReceipt->molblInformations->blcontainers as $blcontainer)
                    @if($loop->first)
                        ...
                        <td rowspan="{{$countContainers}}"><a href="{{url('feederinformations/'.$moneyReceipt->id)}}" class="btn btn-sm btn-dark-green">{{$moneyReceipt->id}} </a></td>
                        <td rowspan="{{$countContainers}}" class="text-left"> {{$moneyReceipt->bolRef ?? ""}} {{$moneyReceipt->extensionNo ? "(Ext-$moneyReceipt->extensionNo)" : null}}</td>
                    @endif
                    <td> {{$blcontainer->contref}}</td>
                    @if($loop->first)
                        <td rowspan="{{$countContainers}}" class="text-left"> {{$moneyReceipt->client->cnfagent}} </td>
                        ...
                        <td rowspan="{{$countContainers}}" style="text-align: right; vertical-align: top">
                            @php($total = 0)
                            @foreach($moneyReceipt->mloMoneyReceiptDetails as $receiptDetails)
                                {{$receiptDetails->amount. '/-'}}@if(!$loop->last) <br>@endif
                        ...
                        <td rowspan="{{$countContainers}}">
                            @if ($moneyReceipt->deliveryOrder)
                                @if ($moneyReceipt->deliveryOrder->id)
                                    <a target="_blank" class="btn btn-sm bg-success p-1 m-1" href="{{ route('mloDoPDF', $moneyReceipt->deliveryOrder->id) }}">DO-{{ $moneyReceipt->deliveryOrder->id }}</a>
                                    <br>
                        ...
                        <td rowspan="{{$countContainers}}">
                            <div class="icon-btn">
                                <nobr>
                                    ...
                                    <a href="{{ url('mlomoneyreceipts/'.$moneyReceipt->id.'/edit') }}" data-toggle="tooltip" title="Edit" class="btn btn-warning"><i class="fas fa-pen"></i></a>
                                    @endcan

                                    <a href="{{ url('mloMoneyReceiptPdf/'.$moneyReceipt->id) }}" data-toggle="tooltip" title="Print" class="btn btn-success"><i class="fas fa-print"></i></a>

                                    @can('mlo-moneyreceipt-delete')
                                    <form action="{{ url('mlomoneyreceipts', [$moneyReceipt->id]) }}" method="POST" data-toggle="tooltip" title="Delete" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger btn-sm delete"><i class="fas fa-trash"></i></button>
                                    </form>
                                    @endcan
                                    <a href="{{url('mlomoneyreceipts/log/'.$moneyReceipt->id)}}" data-toggle="tooltip" title="Log" class="btn btn-dark"><i class="fas fa-history"></i></a>
                                </nobr>
                            </div>
                        </td>
                    @endif
