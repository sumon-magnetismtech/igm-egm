...
class MLOReportController extends Controller
{
    ...
    public function commitmentPDF(){
        $now = now(6);
        $pdf = MPDF::loadView('mlo.reports.commitmentPDF');
    ...
    {
        $feederInfo = Feederinformation::with('mloblInformation.blcontainers', 'mloblInformation.blNotify')->where('id', $feeder_id)->firstOrFail();
        //['contref', 'type', 'status']
        $containers = Blcontainer::with('mloblinformation:id,bolreference')
    ...
    {
        $feederInfo = Feederinformation::with('mloblInformation.blcontainers', 'mloblInformation.blNotify')->where('id', $feeder_id)->firstOrFail();
        $allContainers = Blcontainer::whereIn('mloblinformation_id', $feederInfo->mloblInformation->pluck('id'))->get();
        $containerGroups = $allContainers->groupBy('type')->map(function($item,$key){
            return collect($item)->count();
        });
        return \Barryvdh\DomPDF\Facade::loadView('mlo.reports.ioccontainerPDF', compact('feederInfo', 'containerGroups'))->setPaper('A4', 'landscape')->stream("ioccontainer_feeder_$feeder_id.pdf");
    }


    public function lclContainerList($feeder_id){
        $blinformations = Mloblinformation::with('blcontainers', 'mlofeederInformation')
            ->whereHas('mlofeederInformation', function($q) use($feeder_id){$q->where('id', $feeder_id);})
        ...
        if($blinformations->isNotEmpty()){
            ...
            return \Barryvdh\DomPDF\Facade::loadView('mlo.reports.lclContainerListPDF', compact('blinformations'))->setPaper('A4', 'landscape')->stream("lclContainerList_feeder_$feeder_id.pdf");
        }else{
    ...
    {
        $feederinformation = Feederinformation::
        with('mloblinformation.blcontainers', 'mloblinformation.blNotify','mloblinformation.package')
            ->with(['mloblinformation' => function($q){$q->where('PUloding', '!=', "BDCGP");}])
            ->where('id', $feeder_id)->first();
        $unloadingGroups = $feederinformation->mloblinformation->groupBy('PUloding');

        if($unloadingGroups->isNotEmpty()){
            return \Barryvdh\DomPDF\Facade::loadView('mlo.reports.inboundContainerListPDF', compact('feederinformation','unloadingGroups'))
    ...
    {
        $blinformationGroups = Mloblinformation::whereHas('mlofeederInformation', function($q) use($feeder_id){$q->where('id', $feeder_id);})->where(function($q){$q->where('PUloding', "BDKAM")->orWhere('PUloding', "BDPNG");})->get(['id','feederinformations_id', 'PUloding', 'unloadingName'])->groupBy('PUloding');
        if($blinformationGroups->isNotEmpty()){
            return \Barryvdh\DomPDF\Facade::loadView('mlo.reports.permissionPDF', compact('blinformationGroups'))->stream("permissionPDF_feeder_$feeder_id.pdf", array('Attachment'=>0));
        }else{
    ...
    public function permissionBengaliPDF($feeder_id){
        $blinformationGroups = Mloblinformation::whereHas('mlofeederInformation', function($q) use($feeder_id){$q->where('id', $feeder_id);})->where(function($q){$q->where('PUloding', "BDKAM")->orWhere('PUloding', "BDPNG");})->get(['id','feederinformations_id', 'PUloding', 'unloadingName'])->groupBy('PUloding');
        if($blinformationGroups->isNotEmpty()){
            $pdf = MPDF::loadView('mlo.reports.permissionBengaliPDF', compact('blinformationGroups'));
            $pdf->SetProtection(['copy', 'print'], '', 'pass');
    ...
    {
        $blInformations = Mloblinformation::where('feederinformations_id', $feeder_id)
            ->where('bltypecode', 'hsb')
            ->get();
        if($blInformations->isNotEmpty()){
            return \Barryvdh\DomPDF\Facade::loadView('mlo.reports.arrivalNoticePDF',compact('blInformations'))->stream("arrivalNotices_Feeder_$feeder_id.pdf");
        }else{
    ...
    {
        $feederInfo = Feederinformation::with('mloblInformation.blcontainers', 'country:id,iso,name')->where('id', $feederId)->first();
...
//        dd($feederinformation);
        return \Barryvdh\DomPDF\Facade::loadView('mlo.reports.mlo-bl-print', compact('feederinformation', 'status'))
            ->setPaper('A4', 'landscape')
    ...
    public function inboundPerformanceReport(Request $request)
    {
        ...
        $mloblinformations = Mloblinformation::with('principal','mlofeederInformation:id,feederVessel,voyageNumber,rotationNo,berthingDate', 'blcontainers:id,mloblinformation_id,contref,type', 'blcontainers.containerGroup')
        ...
        $locationWiseContTypesWithCount = $unloadingLocations->map(function($blinformations){
            $container = Blcontainer::with('containerGroup:isocode,group')->whereIn('mloblinformation_id', $blinformations->pluck('id'))->get(['type']);
            return $data = $container->groupBy('containerGroup.group')->map(function($item){
                return collect($item)->count();
            });
        });
        if($reportType == "pdf"){
            return PDF::loadView('mlo.reports.inboundPerformanceReportPDF',compact('mloblinformations', 'unloadingLocations', 'locationWiseContTypesWithCount','dateType','fromDate','tillDate','reportType'))
                ->setPaper('A3', 'landscape')->stream("inbound-performance-report".now()->format('d-m-Y').".pdf");
        }else{
            return view('mlo.reports.inboundPerformanceReport', compact('mloblinformations', 'unloadingLocations', 'locationWiseContTypesWithCount','dateType','fromDate','tillDate','reportType'));
        }
    }
    ...
    public function ladenReport(Request $request)
    {
        ...
        $blInformations = Mloblinformation::with('mloMoneyReceipt', 'blNotify', 'mlofeederInformation', 'blcontainers', 'principal')
...
//        dd($blInformations->toArray());
        if($requestType == "pdf"){
            return PDF::loadView('mlo.reports.ladenReport',compact('blInformations', 'principal', 'duration'))
                ->setPaper('A3', 'landscape')->stream("inbound-performance-report".now()->format('d-m-Y').".pdf");
        }else{
            return view('mlo.reports.ladenReport', compact('blInformations', 'principal', 'duration'));
        }
    }
